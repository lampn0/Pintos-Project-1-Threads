...
thread_start (void) 
{
  ...
  /* my code begins */
  /* Initialize load_avg. */
  load_avg = FP_CONST (0);  /* Convert a value to fixed-point value. */
  /* my code ends */
...
thread_yield (void) 
{
  ...
  /* my code begins */
    list_insert_ordered (&ready_list, &cur->elem, thread_priority_large, NULL);
  /* my code ends */
...
/* If the priority is donated by another thread, it may not change immediately. */
void
thread_set_priority (int new_priority) 
{
  ...
  /* Only update priority and test preemption if new priority is smaller
     and current priority is not donated by another thread. */
...
thread_add_lock (struct lock *lock)
{
  ...
  /* Update priority and test preemption if lock's priority is larger than
  current priority. */
...
{
  enum intr_level old_level = intr_disable ();
  /* Remove lock from the list and update priority. */
...
/*  Update thread's priority. This function only update priority and
    do not update preempt. */
void
